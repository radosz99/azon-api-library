...
class Article:
    def __init__(self, pk, authors, title, journal, year, number='',
                 pages='', note='', issn=''):
        self.pk = pk
        self.authors = []
        for i in range(len(authors)):
    ...
    def get_bibtex(self):
        ...
        if(self.note != '' and self.note is not None):
            bib += f'\n  note =\t"{self.note}",'
        if(self.issn != '' and self.issn is not None):
            bib += f'\n  issn =\t"{self.issn}",'
        bib = bib[0:len(bib)-1]+'\n}\n'
        return bib
...
class InBook:
    def __init__(self, pk, authors, title, pages, publisher, year,
                 address='', note='', issn=''):
        self.pk = pk
        self.authors = []
        for i in range(len(authors)):
...
class Misc:
    def __init__(self, pk, authors='', creators='', title='', note='',
                 _type='', year='', series='', address='', number='',
                 publisher=''):
        self.pk = pk
        self.authors = []
        for i in range(len(authors)):
    ...
    def get_bibtex(self):
        bib = f'@misc{{{self.pk},'
        if(self.authors_string != '' and self.authors_string is not None):
        ...
        if(self.number != '' and self.number is not None):
            bib += f'\n  number =\t"{self.number}",'
        if(self.publisher != '' and self.publisher is not None):
            bib += f'\n  publisher =\t"{self.publisher}",'
        bib = bib[0:len(bib)-1]+'\n}\n'
        return bib
...
class Magazine:
    def __init__(self, pk, title, authors='', creators='', year='',
                 _type='', number='', pages='', address='', note=''):
        self.pk = pk
        self.authors = []
        for i in range(len(authors)):
            author = authors[i]['author']
    ...
    def get_bibtex(self):
        bib = f'@misc{{{self.pk},'
        if(self.creators_string != '' and self.creators_string is not None):
